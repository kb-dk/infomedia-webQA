...
export default defineComponent({
  ...
  methods: {
    ...
    async getNewspapers() {
      ...
      try {
        const newspapersGet = (await axios.get(`/kuana-ndb-api/v1/batches/${this.batch.id}/newspapers`).catch((err) => {
          this.errorMessage  = "Unable to load newspapers";
          console.log(err)
        }))
        const newspapers = newspapersGet.data;
        for (let i = 0; i < newspapers.length; i++) {
          const {data} = await axios.get(`/kuana-ndb-api/v1/batches/${this.batch.id}/newspapers/${newspapers[i].id}/problem-count`).catch((err) => {
          ...
          const newspaperProblemsGet = (await axios.get(`/kuana-ndb-api/v1/batches/${this.batch.id}/newspapers/${newspapers[i].id}/newspaper-problems`).catch(err=>{
        ...
        const {data} = await axios.get(`/kuana-ndb-api/v1/batches/${this.batch.id}/problems-batch`).catch((err) => {
