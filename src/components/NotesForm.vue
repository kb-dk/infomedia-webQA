...
export default defineComponent({
  ...
  methods: {
    async createPost(post: { id: any; note: any; batch_id: any }) {
      ...
      try {
        let url = `/kuana-ndb-api/v1/batches/${this.batch.id}`;
    ...
    async getPosts() {
      ...
      try {
        ...
        switch (this.notesType) {
          case NotesType.BATCHNOTE:
            result = await axios.get(`/kuana-ndb-api/v1/batches/${this.batch?.id}/notes-to-batch`);
            // console.log("BATCHNOTE: " + result?.data)
            break;
          case NotesType.EDITIONNOTE:
            if (this.newspaper) {
              result = await axios.get(`/kuana-ndb-api/v1/batches/${this.batch?.id}/newspapers/${this.newspaper.id}/notes-to-edition`);
              // console.log("EDITIONNOTE: " + result?.data)
            }
            break;
          case NotesType.SECTIONNOTE:
            if (this.newspaper && this.sectiontitle) {
              result = await axios.get(`/kuana-ndb-api/v1/batches/${this.batch?.id}/newspapers/${this.newspaper.id}/notes-to-section?section_title=${this.sectiontitle}`);
              // console.log("SECTIONNOTE: " + result?.data)
            }
            break;
          case NotesType.PAGENOTE:
            if (this.newspaper && this.sectiontitle && this.pagenumber) {
              result = await axios.get(`/kuana-ndb-api/v1/batches/${this.batch?.id}/newspapers/${this.newspaper.id}/notes-to-pages?section_title=${this.sectiontitle}&page_number=${this.pagenumber}`);
